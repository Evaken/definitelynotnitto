...
describe('Stages 7 and 8 vehicle garage',()=>{
  ...
  it('buys and switches cars without losing the previous vehicle colour',()=>{let state=createGarageState('civic-si',100_000);const painted=applyAppearance(state,{...state.appearance,hue:310,wheelStyle:2,components:{...state.appearance.components,wheels:'wheel-mesh'}});expect(painted.ok).toBe(true);if(!painted.ok)return;state=painted.state;const bought=buyCar(state,'rsx-type-s');expect(bought.ok).toBe(true);if(!bought.ok)return;state=bought.state;expect(ownedCarIds(state)).toEqual(['civic-si','rsx-type-s']);const rsx=state.ownedCars.find(car=>car.build.carId==='rsx-type-s')!,selected=selectCar(state,rsx.vehicleId);expect(selected.ok).toBe(true);if(!selected.ok)return;expect(selected.state.build.carId).toBe('rsx-type-s');const civic=selected.state.ownedCars.find(car=>car.build.carId==='civic-si')!,restored=selectCar(selected.state,civic.vehicleId);expect(restored.ok).toBe(true);if(!restored.ok)return;expect(restored.state.appearance).toMatchObject({hue:310,wheelStyle:0,components:{wheels:'wheel-stock'}});expect(restored.state.ownedCars.some(car=>car.build.carId==='rsx-type-s')).toBe(true);});
  it('stores colour independently of performance and resets unsupported visuals',()=>{const state=createGarageState();const appearance={...state.appearance,hue:220,wheelStyle:3,rideHeight:-25,components:{...state.appearance.components,wheels:'wheel-drag'}};const changed=applyAppearance(state,appearance);expect(changed.ok).toBe(true);if(changed.ok){expect(changed.state.appearance).toEqual({...state.appearance,hue:220});expect(changed.state.build).toEqual(state.build);expect(changed.state.cash).toBe(state.cash);}});
});
