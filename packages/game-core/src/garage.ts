...
export function createGarageState(carId='civic-si',cash=10_000):GarageState{return{cash,hasSelectedCar:true,...stockOwnedCar(carId),ownedCars:[],record:{wins:0,losses:0,races:0},transactions:[]};}
/** Testing/live-service onboarding state: the player owns nothing until visiting the showroom. */
export function createEmptyGarageState(cash=1_000_000):GarageState{return{...createGarageState('civic-si',cash),hasSelectedCar:false};}
...
export function buyCar(state:GarageState,carId:string):GarageResult{const car=getCar(carId);if(ownedCarIds(state).includes(carId))return{ok:false,reason:'Car already owned.'};const locked=carUnlockReason(state,carId);if(locked)return{ok:false,reason:locked};if(state.cash<car.price)return{ok:false,reason:'Not enough cash.'};const transactions=transaction(state,'car',-car.price,`${car.manufacturer} ${car.displayName}`);if(!state.hasSelectedCar)return{ok:true,state:{...state,...stockOwnedCar(carId),cash:state.cash-car.price,hasSelectedCar:true,transactions}};return{ok:true,state:{...state,cash:state.cash-car.price,ownedCars:[...state.ownedCars,stockOwnedCar(carId)],transactions}};}
export function selectCar(state:GarageState,carId:string):GarageResult{if(!state.hasSelectedCar)return noCar();if(carId===state.build.carId)return{ok:false,reason:'Car already selected.'};const selected=state.ownedCars.find(car=>car.build.carId===carId);if(!selected)return{ok:false,reason:'Car not owned.'};const current:OwnedCarState={build:state.build,ownedPartIds:state.ownedPartIds,tune:state.tune,condition:state.condition,appearance:state.appearance};return{ok:true,state:{...state,...selected,ownedCars:[...state.ownedCars.filter(car=>car.build.carId!==carId),current]}};}
