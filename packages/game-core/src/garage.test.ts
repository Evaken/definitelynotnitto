...
describe('Stage 3 garage',()=>{
  ...
  it('stores the complete normal roster as independent owned cars',()=>{let state=createEmptyGarageState();const normal=[...CARS.values()].filter(car=>!car.special);for(const car of normal)state=succeed(buyCar(state,car.id)).state;expect(normal.length).toBeGreaterThanOrEqual(10);expect(ownedCarIds(state)).toHaveLength(normal.length);expect(new Set(ownedCarIds(state)).size).toBe(normal.length);expect(state.hasSelectedCar).toBe(true);const last=normal.at(-1)!,instance=state.ownedCars.find(item=>item.build.carId===last.id)!;state=succeed(selectCar(state,instance.vehicleId)).state;expect(state.build.carId).toBe(last.id);expect(state.ownedCars).toHaveLength(normal.length-1);});
  it('allows two independently identified copies of the same model',()=>{let state=createEmptyGarageState();state=succeed(buyCar(state,'civic-si','civic-a')).state;state=succeed(buyCar(state,'civic-si','civic-b')).state;expect(ownedCarIds(state)).toEqual(['civic-si','civic-si']);expect(state.selectedVehicleId).toBe('civic-a');state=succeed(selectCar(state,'civic-b')).state;expect(state.selectedVehicleId).toBe('civic-b');expect(state.ownedCars[0]?.vehicleId).toBe('civic-a');});
  // "Universal" was this test's own word, from when the roster was one car.
  // PROJECT_SPEC 144 lists "Compatible cars / engines" as a field on a part, so
  // fitment restrictions are intended -- they simply had nothing to restrict
  // against until Stages 7-8 brought twelve more cars. The blower parts are the
  // only ones that name cars; see data/parts/fitment.test.ts.
  it('contains 25-40 unique upgrade parts, near enough all of them universal',()=>{const parts=partList();expect(parts.length).toBeGreaterThanOrEqual(25);expect(parts.length).toBeLessThanOrEqual(40);expect(new Set(parts.map(p=>p.id)).size).toBe(parts.length);expect(parts.filter(p=>p.compatibleCarIds.length>0).map(p=>p.id).sort()).toEqual(['race-supercharger','street-supercharger','supercharger-bracket']);});
