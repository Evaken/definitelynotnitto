...
export interface Account {id:string;username:string;passwordHash:string;salt:string;garage:GarageState;raceHistory?:TimingSlip[];disabled?:boolean;createdAt:string;}
export interface Session {token:string;accountId:string;expiresAt:number;}
export interface LockedRun {vehicleId:string;carId:string;garageSnapshot:GarageState;tune:GarageState['tune'];timeline:InputTimeline;slip:TimingSlip;stress:number;}
