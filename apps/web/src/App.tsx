...
import { NoCarScreen } from './screens/NoCarScreen.js';
...
export function App() {
  ...
  const [shopEntryGroup,setShopEntryGroup]=useState<WorkshopGroupId|null>(null);
  const [garageResume,setGarageResume]=useState<{view:'setup'|'tune';group:WorkshopGroupId}|null>(null);
  ...
  const navigate=useCallback((next:ScreenId)=>{if(screen==='track'&&next!=='track'&&pendingStress.current>0){const stress=pendingStress.current;pendingStress.current=0;if(!onlineToken)setGarage(previous=>applyPassStress(previous,stress));}if(next==='garage')setGarageResume(null);if(next==='parts')setShopEntryGroup(null);setShopMessage('');setScreen(next);},[onlineToken,screen]);
  const startCpuRace=useCallback((difficulty:CpuDifficulty)=>{if(!garage.hasSelectedCar){setShopMessage('Purchase a car from the Showroom first.');setScreen('showroom');return;}const opponent=CPU_OPPONENTS[difficulty];setCpuRace({difficulty,name:opponent.name,slip:runCpuOpponent(difficulty,Math.floor(Math.random()*0x7fffffff)),settled:false});setRaceHistory([]);setScreen('track');},[garage.hasSelectedCar]);
  ...
  const purchase=async(id:string):Promise<boolean>=>{if(!onlineToken){const result=purchaseAndFitPart(garage,id);apply(result,'Purchase complete. Component installed.');return result.ok;}setShopMessage('Saving to online garage…');try{const state=await api.garage(onlineToken,{type:'purchase-part',id});setGarage(state);setRaceHistory([]);setShopMessage('Purchase complete. Component installed.');return true;}catch(error){setShopMessage(error instanceof Error?error.message:'Online action failed.');return false;}};
  ...
  return (
    <div className="shell">
      ...
      {screen==='main'?<MainScreen state={garage} onRace={startCpuRace} onVisitShowroom={()=>setScreen('showroom')} {...(onlineProfile?{playerName:onlineProfile.username}:{})} accountPanel={accountPanel}/>:screen === 'track' ? garage.hasSelectedCar?<RaceTrackScreen car={car} tune={garage.tune} appearance={garage.appearance} fittedPartCount={garage.build.fittedPartIds.length} initialHistory={raceHistory} onHistoryChange={updateRaceHistory} onPassStress={recordStress} onRecorded={recordRun} {...(cpuRace?{opponent:cpuRace,onCompleted:completeCpuRace}:{})}/>:<NoCarScreen title="No Car Selected" message="Buy a car before you enter the race track. The exact selected build will appear at the staging line." onVisitShowroom={()=>setScreen('showroom')}/>:screen==='garage'?<GarageScreen key={`${garageResume?.view??'overview'}-${garageResume?.group??'none'}`} state={garage} car={car} history={raceHistory} message={shopMessage} onVisitShop={group=>{setShopEntryGroup(group);setScreen('parts');}} onVisitShowroom={()=>setScreen('showroom')} onFit={fit} onRemove={remove} onTune={tune} onRepair={repair} onAppearance={appearance} onSelect={chooseCar} {...(garageResume?{initialView:garageResume.view,initialGroup:garageResume.group}:{})}/>:screen==='parts'?garage.hasSelectedCar?<PartsShopScreen key={shopEntryGroup??'home'} state={garage} message={shopMessage} initialGroup={shopEntryGroup} onPurchaseAndFit={purchase} onReturnGarage={group=>{setGarageResume({view:'setup',group});setScreen('garage');}} onVisitDyno={group=>{setGarageResume({view:'tune',group});setScreen('garage');}} onVisitRace={()=>setScreen('track')}/>:<NoCarScreen title="No Vehicle to Modify" message="The Speedshop installs parts onto the currently selected car. Choose a vehicle first." onVisitShowroom={()=>setScreen('showroom')}/>:screen==='showroom'?<ShowroomScreen state={garage} message={shopMessage} onBuy={purchaseCar} onSelect={chooseCar}/>:screen==='challenge'?<ChallengeScreen token={onlineToken} profile={onlineProfile} lastRun={lastRecordedRun}/>:screen==='team'?<TeamScreen token={onlineToken} profile={onlineProfile} lastRun={lastRecordedRun}/>:screen==='community'?<CommunityScreen token={onlineToken} profile={onlineProfile}/>: (
