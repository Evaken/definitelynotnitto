...
import {factoryPaintAppearance,type Appearance} from '@nitto/game-core';
...
export function CivicLayeredPortrait({appearance,className=''}:{appearance:Appearance;className?:string}){
  ...
  useEffect(()=>{let current=true;const canvas=canvasRef.current;if(!canvas)return;setReady(false);void renderCivicToCanvas(canvas,'garage',factoryPaintAppearance(appearance)).then(()=>current&&setReady(true));return()=>{current=false;};},[appearance]);
