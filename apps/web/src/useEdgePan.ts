...
export function useEdgePan<T extends HTMLElement>(){
  ...
  const onPointerMove=(event:PointerEvent<T>)=>{if(event.pointerType==='touch')return;const formControl=(event.target as HTMLElement).closest('select,input,a');if(formControl){stop();return;}const bounds=event.currentTarget.getBoundingClientRect();velocity.current=edgePanVelocity(event.clientX-bounds.left,bounds.width);if(velocity.current!==0&&frame.current===null)frame.current=requestAnimationFrame(tick);if(velocity.current===0)stop();};
