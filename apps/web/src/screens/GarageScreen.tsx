...
export function GarageScreen({state,car,message,onVisitShop,onVisitShowroom,onFit,onRemove,onTune,onRepair,onAppearance,onSelect,initialView='overview',initialGroup='intake'}:{state:GarageState;car:Car;history:readonly TimingSlip[];message:string;onVisitShop:(group:WorkshopGroupId)=>void;onVisitShowroom:()=>void;onFit:(id:string)=>void;onRemove:(id:string)=>void;onTune:(tune:Tune)=>void;onRepair:()=>void;onAppearance:(appearance:Appearance)=>void;onSelect:(id:string)=>void;initialView?:GarageView;initialGroup?:WorkshopGroupId}){
  ...
  const activeModificationDepartment=modificationDepartmentFor(group,category);
  const chooseSystem=(system:ModificationSystemSlot)=>{setGroup(system.groupId);setCategory(system.category);setSelectedId('');};
  const chooseModificationDepartment=(departmentId:(typeof MODIFICATION_DEPARTMENTS)[number]['id'])=>chooseSystem(firstModificationSystem(departmentId));

  const changeDepartment=(department:'modifications'|'tune'|'paint'|'maintenance')=>setView(department==='modifications'?'setup':department);
  if(view==='tune')return <div className="screen screen--workshop screen--garage-department"><WorkshopFrame cash={state.cash} carLabel={carLabel} showDepartments activeDepartment="tune" onDepartmentChange={changeDepartment} onBack={()=>setView('overview')}><TuneDynoPanel car={car} tune={state.tune} appearance={state.appearance} message={message} onApply={onTune}/></WorkshopFrame></div>;
  ...
  if(view==='maintenance')return <div className="screen screen--workshop screen--garage-department"><WorkshopFrame cash={state.cash} carLabel={carLabel} showDepartments activeDepartment="maintenance" onDepartmentChange={changeDepartment} onBack={()=>setView('overview')}><section className="maintenance-bay garage-service-view"><header className="garage-service-header"><div><span>Service Bay 04</span><h2>Workshop Inspection</h2></div><small>Post-race condition report</small></header><div className="maintenance-layout"><CarBay className="car-bay--inspection" carId={car.id} title={`${car.year} ${car.displayName}`} subtitle="Mechanical inspection" badge="SERVICE BAY" fittedParts={fittedParts} appearance={state.appearance}/><div className="condition-card"><div className="condition-dial" style={{'--condition':`${state.condition*3.6}deg`} as React.CSSProperties}><strong>{state.condition.toFixed(1)}%</strong><span>Condition</span></div><div><h3>{state.condition>85?'Race ready':state.condition>55?'Service recommended':'Critical wear'}</h3><p>Race stress reduces engine output until the car is serviced.</p><dl><div><dt>Power retained</dt><dd>{Math.round(70+state.condition*.3)}%</dd></div><div><dt>Repair estimate</dt><dd>${repairCost(state).toLocaleString()}</dd></div></dl><button type="button" className="workshop-action" disabled={repairCost(state)===0||state.cash<repairCost(state)} onClick={onRepair}>Authorise Repairs</button></div></div></div><p className={`workshop-message${message?' workshop-message--active':''}`}>{message||'Inspection results update after every completed pass.'}</p></section></WorkshopFrame></div>;
  ...
  return <div className="screen screen--workshop screen--garage-department screen--garage-setup"><WorkshopFrame cash={state.cash} carLabel={carLabel} showDepartments activeDepartment="modifications" onDepartmentChange={changeDepartment} onBack={()=>setView('overview')}>
    <nav className="modification-department-strip" aria-label="Workshop departments">
      {MODIFICATION_DEPARTMENTS.map(department=><button key={department.id} data-sound="select" type="button" aria-pressed={activeModificationDepartment.id===department.id} className={activeModificationDepartment.id===department.id?'active':''} onClick={()=>chooseModificationDepartment(department.id)}><span>{department.label}</span><small>{department.systems.length} system{department.systems.length===1?'':'s'}</small></button>)}
    </nav>
    <div className="workshop__stage workshop__stage--simple">
      <div className={`workshop__visual workshop__visual--hoist workshop__visual--focus-${activeModificationDepartment.focus}`}>
        <CarBay carId={car.id} title={`${car.manufacturer} ${car.displayName}`} subtitle={`${Math.round(peakHp)} HP · ${Math.round(torque.torqueNm)} NM · ${Math.round(car.chassis.massKg)} KG · ${state.build.fittedPartIds.length?`${state.build.fittedPartIds.length} MODIFIED`:'STOCK'}`} badge="" fittedParts={fittedParts} appearance={state.appearance}/>
        <div className="workshop-zone-light" aria-hidden="true"><i/><span>{activeModificationDepartment.label}</span></div>
      ...
      <section className="workshop__inventory workshop__inventory--simple garage-components">
        <header><span>{activeModificationDepartment.label}</span><small>{categoryLabel(category)}</small></header>
        <nav className="workshop-system-slots" aria-label={`${activeModificationDepartment.label} systems`}>
          {activeModificationDepartment.systems.map(system=><button key={`${system.groupId}-${system.category}`} data-sound="select" type="button" className={group===system.groupId&&category===system.category?'active':''} aria-pressed={group===system.groupId&&category===system.category} onClick={()=>chooseSystem(system)}><i/>{system.label}</button>)}
        </nav>
...
function PaintShop({car,appearance,message,onApply}:{car:Car;appearance:Appearance;message:string;onApply:(appearance:Appearance)=>void}){
  ...
  return <section className="paint-shop paint-shop--colour-only garage-service-view"><header className="garage-service-header"><div><span>Paint Booth 03</span><h2>Body Colour</h2></div><small>Live preview · saved to this vehicle</small></header><div className="paint-shop__body"><div className="paint-preview"><CarBay className="car-bay--paint-booth" carId={car.id} title={`${car.year} ${car.manufacturer} ${car.displayName}`} subtitle="Factory bodywork · colour preview" badge="COLOUR MATCH" appearance={draft}/><div className="paint-swatches">{[0,28,55,120,195,245,310].map(hue=><button key={hue} type="button" aria-label={`Paint hue ${hue}`} aria-pressed={Math.abs(draft.hue-hue)<1} style={{background:`hsl(${hue} 78% 52%)`}} onClick={()=>chooseHue(hue)}/>)}</div></div><div className="paint-controls paint-controls--colour-only"><span>Mixing console</span><h3>Factory Body Colour</h3><p>Preview the finish under booth lighting, then apply it to this vehicle.</p><label><span>Body colour</span><input type="range" min="0" max="360" step="1" value={draft.hue} onChange={event=>chooseHue(Number(event.target.value))}/><output>{Math.round(draft.hue)}°</output></label><div className="paint-colour-summary"><i style={{background:`hsl(${draft.hue} 78% 52%)`}}/><div><span>Current mix</span><strong>Hue {Math.round(draft.hue)}°</strong></div></div><button type="button" className="workshop-action" onClick={()=>onApply(factoryPaintAppearance(draft))}>Apply Body Colour</button></div></div><p className={`workshop-message${message?' workshop-message--active':''}`}>{message||'The saved colour follows this exact car through the Garage, Workshop and Race Track.'}</p></section>;
}
