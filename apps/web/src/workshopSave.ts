import { createGarageState, getCar, partList, stockAppearance, stockTune, validateTune, type Appearance, type GarageState, type OwnedCarState, type Tune } from '@nitto/game-core';
...
export function parseWorkshopSave(raw:string|null):GarageState|null{
  ...
  try{
    ...
    return{cash:Math.round(candidate.cash),ownedPartIds:owned,build:{carId:candidate.build.carId,fittedPartIds:fitted},tune,condition,appearance,ownedCars,record,transactions};
  }catch{return null;}
}
