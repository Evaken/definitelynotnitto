import { STAGING, TRACK_MARKS, displayGear, engineRpm, metresToFeet, msToMph, SIM_HZ, } from '@nitto/game-core';
import { CANVAS_HEIGHT, CANVAS_WIDTH, CAR_SCREEN_X, COLORS, HORIZON_Y, HUD_HEIGHT, PX_PER_M, TRACK_BOTTOM, TRACK_Y, worldToScreen, } from './layout.js';
 ...
 */
export function drawRace(ctx, state) {
...
function drawDistanceMarkers(ctx, camera) {
    const majors = [
 ...
 */
function drawBeams(ctx, camera, state) {
    const beams = [
...
function drawHud(ctx, state) {
    ...
    ctx.stroke();
    const elapsed = state.clockStartTick === null ? 0 : Math.max(0, (state.tick - state.clockStartTick) / SIM_HZ);
...
function readout(ctx, x, label, value) {
    ctx.textAlign = 'left';
...
function drawTacho(ctx, x, y, width, height, state) {
...
function drawDistanceStrip(ctx, state) {
...
//# sourceMappingURL=drawRace.js.map